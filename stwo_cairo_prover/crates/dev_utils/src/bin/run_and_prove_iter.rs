//! CPU run_and_prove with --iterations support. Mirrors run_and_prove_cuda.
...
fn main() -> Result<()> {
    ...
    tracing_subscriber::fmt()
        ...
        .init();
    let _span = span!(Level::INFO, "run_and_prove_iter").entered();
    ...
    for iter in 0..args.iterations {
        ...
        let t0 = Instant::now();
        let proof = prove_cairo::<Blake2sMerkleChannel>(pi, prover_params.clone())
    ...
    eprintln!(
        "[summary] adapt: {:?} prove_cold: {:?} prove_warm: {:?}",
        t_adapt, t_prove_first, t_prove_last
    );
